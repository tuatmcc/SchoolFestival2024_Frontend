...
CREATE TABLE public.players (
  ...
  UNIQUE (user_id, battle_result_id),
  -- team_idから参照されるbattle_result_idと一致することを保証する
  FOREIGN KEY (team_id, battle_result_id) REFERENCES public.teams (id, battle_result_id) ON DELETE CASCADE
);


-- データ更新時に更新日時を更新するトリガー
CREATE TRIGGER
  update_battle_result_timestamp
...
CREATE INDEX idx_players_battle_result_id ON public.players (battle_result_id);

CREATE INDEX idx_teams_battle_result_id ON public.teams (battle_result_id);
...
-- 参照のみ全ユーザーに許可
CREATE POLICY
  "Can read everyone's battle result"
